            ...
            if ( rInf.next() ) {
                ...
                if( rInf.getInt("cantidad") < nUnd ){
                    ...
                    if ( lUpdateInsert == true ) {  // Ya esta en la cesta realizar Update en cesta.
                        cUpdate = "UPDATE CESTA SET cantidad_cesta = cantidad_cesta + ? WHERE id_cliente = ? AND id_artcesta = ?";
                        ...
                        if (nRetorno2==1) {
                            JOptionPane.showMessageDialog(null,"¡¡ ACTUALIZACION DE LA CESTA!!  | Articulo [ "+cTit+" - "+cDes+" ], Grabada.");
                            response.sendRedirect("ServletListaCesta");
                        }
                    }else{  // No existe en la cesta, realizar Insert en cesta.
                        cInsert = "INSERT INTO CESTA (id_cliente,id_artcesta, precio_cesta,cantidad_cesta) VALUES ( ?,?,?,? )";
                        ...
                        int nRetorno2 = act.executeUpdate();
                        if (nRetorno2==1) {
                            JOptionPane.showMessageDialog(null,"¡¡ ARTICULO AÑADIDO A LA CESTA!!\nArticulo [ "+cTit+" - "+cDes+" ,  Unidades = " + nUnd + " ]\n, Grabado.");
                            response.sendRedirect("ServletListaCesta");
                        }
                       }
                    }
                }else{ 
                    response.sendRedirect("No_Existe.jsp");
                }
